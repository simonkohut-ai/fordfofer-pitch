...
Write-Host ""
Write-Host "=== Root Static Files Check ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "=== SEO Essentials Check ===" -ForegroundColor Cyan
Write-Host ""

$seoUrls = @(
    "https://www.golocapo.com/robots.txt",
    "https://www.golocapo.com/sitemap.xml"
)

$allSeoPass = $true

foreach ($url in $seoUrls) {
    try {
        $response = Invoke-WebRequest -Uri $url -Method Head -TimeoutSec 15 -UseBasicParsing
        $status = $response.StatusCode
...
if (-not $allFilesExist -or -not $allRootFilesExist) {
    Write-Host "❌ FAIL - Files missing. Cannot proceed." -ForegroundColor Red
    exit 1
}

# Check all URLs pass
if ($allRewritePass -and $allBrandPass -and $allSeoPass) {
    Write-Host "✅ SUCCESS - All rewrite URLs, brand assets, and SEO essentials return HTTP 200" -ForegroundColor Green
    exit 0
} else {
    ...
    if (-not $allSeoPass) {
        Write-Host "   SEO essentials failed" -ForegroundColor Yellow
    }
